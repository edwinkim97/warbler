...
from models import db, Message, User, Follows, MessageLikes
...
class UserViewTestCase(TestCase):
    ...
    def setup_likes(self):
        ...
        l1 = MessageLikes(user_id=self.testuser_id, message_id=9876)
    ...
    def test_add_like(self):
        ...
        with self.client as c:
            ...
            resp = c.post("/messages/1984", data={"message_id":"1984", "redirect": "messages/1984"}, follow_redirects=True)
            self.assertEqual(resp.status_code, 200)

            likes = MessageLikes.query.filter(MessageLikes.message_id==1984).all()
    ...
    def test_remove_like(self):
        ...
        l = MessageLikes.query.filter(
            MessageLikes.user_id==self.testuser_id and MessageLikes.message_id==m.id
        ).one()
        ...
        with self.client as c:
            ...
            resp = c.post(f"/messages/{m.id}", data={"message_id":m.id, "redirect": f"messages/{m.id}"},follow_redirects=True)
            self.assertEqual(resp.status_code, 200)

            likes = MessageLikes.query.filter(MessageLikes.message_id==m.id).all()
    ...
    def test_unauthenticated_like(self):
        ...
        like_count = MessageLikes.query.count()
        ...
        with self.client as c:
            ...
            self.assertEqual(like_count, MessageLikes.query.count())
